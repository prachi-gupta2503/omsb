...
@Component(immediate = true, service = EducationDetailsUtil.class)
public class EducationDetailsUtil {

	public EducationDetailItem getEducationDetailItemsByPersonId(String portalURL, long groupId, long personId) {
		_logger.info("getEducationDetailItemsByPersonId Invoked :::");
		StringBuilder sbURL = new StringBuilder(
				portalURL + LRObjectURL.REG_EDUCATION_DETAIL_URL + CommonConstants.SCOPES + StringPool.SLASH + groupId);
		if (personId != 0) {
			try {
		...
		_logger.info("getEducationDetailItemsByPersonId Exit ::: " + sbURL.toString());
	...
	private <T> T getItems(String url, Class<T> clazz) {
		...
		_logger.info("Education Detail object response ::: " + response);
