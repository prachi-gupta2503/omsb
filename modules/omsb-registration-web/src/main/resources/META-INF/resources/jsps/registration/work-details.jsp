			...
			<div class="col-12 login-right">
				<div class="omsb-card ">
					...
					<div class="reg_step3" id="reg_step3">
						<div class="alert alert-light alert-success-text d-none" id="educationDetailSuccess" role="alert">
								...
								<div class="alert-text">
									<h4 class="alert-heading">
										<span> <liferay-ui:message
												key="education-details-are-saved-successfully" /></span>
									</h4>
								</div>
							</div>
						</div>
												...
												<div class="row ">
													...
													<div class="col-lg-4 col-md-6 d-none" id="worksectorTypediv_1">
														<div class="form-group">
															...
															<input type="text" name="<portlet:namespace/>worksectortypeother_1" id="worksectortypeother_1" value=" " class="form-control">
														</div>
													</div>
													...
													<div class="col-lg-12 col-md-12">
														<div class="form-group">
															...
															<div class="omsb-custom-html-file custom-file">
																<input id="staffIdCard_1" name="<portlet:namespace/>staffIdCard_1" type="file" label="" class="form-control custom-file-input" onchange="validateFile(this.id,'file-size-error_1','<liferay-ui:message key="please-select-pdf-jpg-png-file-only" />')">
													...
													<c:choose>
														<c:when test="${not empty primaryworkDetail.getWorkSectorTypeOther()}">
															<div class="col-lg-4 col-md-6 " id="worksectorTypediv_1">
																<div class="form-group">
																	<label class="control-label "><liferay-ui:message key="primary-workplace-sector-type-other" /></label>
																	<input type="text" name="<portlet:namespace/>worksectortypeother_1" id="worksectortypeother_1" value="${primaryworkDetail.getWorkSectorTypeOther()}" class="form-control">
																</div>
															</div>
														</c:when>
														<c:otherwise>
						...
						<div class="row ">
							...
							<div class="col-lg-4 col-md-6 d-none" id="worksectorTypediv_2">
										<div class="form-group">
											<label class="control-label "><liferay-ui:message key="secondary-workplace-sector-type-other" /></label>
											 <input type="text" name="<portlet:namespace/>worksectortypeother_2" id="worksectortypeother_2" value="" class="form-control">
										</div>
							</div>
							...
							<div class="col-lg-12 col-md-12">
								<div class="form-group">
									<label class="control-label"><liferay-ui:message key="staff-card-id" /></label>
									<div class="omsb-custom-html-file custom-file">
										<input id="staffIdCard_2" name="<portlet:namespace/>staffIdCard_2" type="file" label="" class="form-control custom-file-input"  onchange="validateFile(this.id,'file-size-error_2','<liferay-ui:message key="please-select-pdf-jpg-png-file-only" />')">
									...
									<c:choose>
										<c:when test="${not empty secondaryWorkDetail.getWorkSectorTypeOther()}">
											<div class="col-lg-4 col-md-6 " id="worksectorTypediv_2">
												<div class="form-group">
													<label class="control-label "><liferay-ui:message key="secondary-workplace-sector-type-other" /></label>
													 <input type="text" name="<portlet:namespace/>worksectortypeother_2" id="worksectortypeother_2" value="${secondaryWorkDetail.getWorkSectorTypeOther()}" class="form-control">
												</div>
											</div>
										</c:when>
										<c:otherwise>
					 		...
					 		<div class="d-flex align-items-center">
						 		<div class="form-group">
									...
									<div class="custom-file">
										<input type="file" name="<portlet:namespace/>cvDocument" id="cvDocument" class="form-control custom-file-input" onchange="validateDocumentFile(this.id,'errorContainer-cvDocument','<liferay-ui:message key="please-upload-pdf-doc-docx-file-only" />')">
										<label class="custom-file-label" for="cvDocument">
											${cvDocument}
										</label>
									</div>
								</div>
								<div style="width:140px" class="text-right">
									<c:if test="${not empty cvDocumentURL}">
											<a href="${cvDocumentURL}" class="btn view_btn text-danger" target="_blank"><img src="<%=themeDisplay.getPathThemeImages()%>/svg/view_icon.svg" alt=""><liferay-ui:message key="view" /></a>
									</c:if>
								</div>
							</div>
...
<liferay-ui:success key="success-work-detail" message="work-detail-successfully-added" />
	<script>
		...
		$( document ).ready(function() {
				...
				var isDeclared='${isDeclared}';
				var isAgreed ='${isAgreed}';
				var isPrimaryWorkDetail='${not empty primaryworkDetail}';
				console.log("isPrimaryWorkDetail ::::",isPrimaryWorkDetail);
				console.log("isDeclared ::::",isDeclared);
				console.log("isAgreed ::::",isAgreed);
				if(isDeclared=='true' && isAgreed=='true' && isPrimaryWorkDetail=='false'){
					console.log("inside isDeclared ture ::::");
					console.log("inside isAgreed ture ::::");
					$('#work-detail-main').addClass('d-none');
					$('#employed_no').attr('checked', 'checked');
					$('#employed_yes').removeAttr('checked');
				}
		...
		function isValidVerificationField(index){
			var errorMessages = [];
		   	console.log("inside validVerification field function");
			var workSectorTypeId=$("#workSectorType_"+index).val();
			if(!workSectorTypeId || workSectorTypeId===""){
				errorMessages.push("<liferay-ui:message key='please-enter-work-sector-type' />");
				$("#work-sector-type-error_"+index).html("<liferay-ui:message key='please-enter-work-sector-type' />");
			} else{
				$("#work-sector-type-error_"+index).html("");
			}
			
			var workSector=$("#worksector_"+index).val();
			var workSectorparentClass=$('#worksector_'+index).parent().attr('class');
			//var workSectorparentClass=$('#worksector_'+index).parent().find('d-none');
			...
			if(!workSectorparentClass.includes('d-none')){
				...
				if(!workSector || workSector===""){
					...
					if(index==1){
						...
						$("#work-sector-error_"+index).html("<liferay-ui:message key='please-enter-work-sector' />");
					}else if(index==2){
						errorMessages.push("<liferay-ui:message key='please-enter-secondary-work-sector' />");
						$("#work-sector-error_"+index).html("<liferay-ui:message key='please-enter-secondary-work-sector' />");
					}
				} else{
					$("#work-sector-error_"+index).html("");
				}
			}
			
			var wilayats=$("#wilayats_"+index).val();
			if(!wilayats || wilayats===""){
				console.log("inside if conditon worksector :  work-sector-error_");
				
				if(index==1){
					$("#location-error_"+index).html("<liferay-ui:message key='please-enter-work-sector-location' />");
					errorMessages.push("<liferay-ui:message key='please-enter-work-sector-location' />");
				}else if(index==2){
					$("#location-error_"+index).html("<liferay-ui:message key='please-enter-secondary-work-sector-location' />");
					errorMessages.push("<liferay-ui:message key='please-enter-secondary-work-sector-location' />");
				}
			} else{
				$("#location-error_"+index).html("");
			}
			
			var designations=$("#designations_"+index).val();
			if(!designations || designations ===""){
				errorMessages.push("<liferay-ui:message key='please-enter-designation' />");
				$("#designation-error_"+index).html("<liferay-ui:message key='please-enter-designation' />");
			} else{
				$("#designation-error_"+index).html("");
			}
			
			var cvDocumentId = '${cvDocumentId eq 0 || empty cvDocumentId}';
			var cvDocument = document.getElementById("cvDocument").files[0];
			if (!cvDocument) {
				if(cvDocumentId=='true'){
					errorMessages.push("<liferay-ui:message key='please-upload-cvDocument' />");
					document.getElementById("errorContainer-cvDocument").textContent = "<liferay-ui:message key='please-upload-cvDocument' />";
				}
			} else{
				var cvDocumentName=cvDocument.name;
				var ext = cvDocumentName.substr(cvDocumentName.lastIndexOf("."));
				console.log(cvDocumentName);
				var allowedExtReg = /(\.pdf|\.docx|\.doc)$/i;
				...
				if(allowed){
					...
					if (size > 1) {
			    	 ...
			    	 document.getElementById("errorContainer-cvDocument").textContent = "";
			    	 document.getElementById("errorContainer-cvDocument").textContent ="<liferay-ui:message key='please-upload-pdf-doc-docx-file-only' />";
			    	 errorMessages.push("<liferay-ui:message key='please-upload-pdf-doc-docx-file-only' />");
			     }
			}
				...
				if(!isEdit){
			    	...
			    	var allowedExtensionsRegex = /(\.jpg|\.jpeg|\.png|\.pdf)$/i;
			     	...
			     	if(isAllowed){
			    		...
			    		document.getElementById("file-size-error_1").textContent ="<liferay-ui:message key='please-select-pdf-jpg-png-file-only' />";
			    	 	errorMessages.push("<liferay-ui:message key='please-select-pdf-jpg-png-file-only' />");
			     	}
				} else{
					isEdit=false;
					document.getElementById("file-size-error_1").textContent = "";
				...
				if(!isEdit){
					...
					var allowedExtensionsRegex_2 = /(\.jpg|\.jpeg|\.png|\.pdf)$/i;
			     	...
			     	if(isAllow){
			    		...
			    		document.getElementById("file-size-error_2").textContent ="<liferay-ui:message key='please-select-pdf-jpg-png-file-only' />";
			    	 	errorMessages.push("<liferay-ui:message key='please-select-pdf-jpg-png-file-only' />");
					}
				} else{
					isEdit=false;
	...
	function saveWorkDetail(button){
			...
			if(!${themeDisplay.isSignedIn()}){
				if(isEmployed==1){
					var isPrimaryWorkDetails=isValidVerificationField("1");
					var isSecondaryWorkDetails=true;
					var workSectorTypeId2=$("#workSectorType_2").val();
					console.log("workSectorTypeId2 ::::::",workSectorTypeId2);
					if(workSectorTypeId2>0 || workSectorTypeId2 !=""){
						isSecondaryWorkDetails=isValidVerificationField("2");
					}else{
					...
					console.log("isPrimaryWorkDetails :::",isPrimaryWorkDetails);
					console.log("isPrimaryWorkDetails :::",isSecondaryWorkDetails);
					
					
					if(isPrimaryWorkDetails && isSecondaryWorkDetails){
						...
						if(button=='next'){
							document.getElementById("<portlet:namespace/>isNext").value = "true";
							 ...
							 console.log("Tab change to role service");
						}
						/* document.getElementById("work-detail-form").submit(); */
						if(isEducationAndEmploymentDetailsSaved()){
							saveWorkDetailData();
						}else{
							setTimeout(function() {
								removeLoader();
		...
		function saveWorkDetailData(){
			var declare2Data = document.getElementById('declare2');
			declare2Data.checked=true;
			var declare1Data = document.getElementById('declare1');
			declare1Data.checked=true;
			let form =$("#work-detail-form")[0];
			//console.log(form);
			...
			complete: function(data) {
			        //console.log("Complete Data, " +data.responseText);
					...
					$('#work-details').addClass('d-none');
					$('#role-service').addClass('d-none');
					$("#workDetailSuccess").removeClass("d-none");
					setTimeout(function() {
						removeLoader();
					}, 200);
		       },
			...
			if(isSignedUser){
				...
				$.ajax({
					...
					success : function(data) {
						...
						showContent(false, !response.isEducationDetailsExist, false, false);
						if(response.isEducationDetailsExist){
							isExist = true;
						}
					},
				})
		...
		function showDesignationOther(id,errorId,errorMessage){
							 ...
							 $.ajax({
									...
									success : function(data) {
										...
										if(field=="first"){
											console.log("if");
											if(options.length>0){
												console.log("length greater first");
											...
											console.log("else");
											if(options.length>0){
												console.log("length greater second");
												$("#div-first-sub-sector_"+substring).removeClass("d-none");
	        ...
	        function validateDocumentFile(id, errorId, errorMessage){
	        	   
	        	var filename = document.getElementById(id).files[0].name;
	            var extension = filename.substr(filename.lastIndexOf("."));
	            var allowedExtensionsReg = /(\.pdf|\.doc|\.docx)$/i;
	            var isAllowed = allowedExtensionsReg.test(extension);
