...
public class AppealWrokflowUtil {
	...
	public WorkflowInstance getWorkflowInstace(String className, ThemeDisplay themeDisplay, long classPK)
	...
	public void assignWorkflowToUser(ThemeDisplay themeDisplay, WorkflowInstance instance, long workflowTaskId)
			throws PortalException {
	...
	public void completeWorkflowTask(ThemeDisplay themeDisplay, WorkflowInstance instance, long workflowTaskId,
			String comments, String transitionName) throws PortalException {
	...
	public void getWorkflowTask(String className, ThemeDisplay themeDisplay, String comments, PortletRequest request) {
		try {
			Role role = null;
			WorkflowInstanceLink instanceLink = WorkflowInstanceLinkLocalServiceUtil.getWorkflowInstanceLink(
					themeDisplay.getCompanyId(), themeDisplay.getScopeGroupId(), className, 47875);
			WorkflowInstance instance = WorkflowInstanceManagerUtil.getWorkflowInstance(themeDisplay.getCompanyId(),
					instanceLink.getWorkflowInstanceId());

			List<Integer> assignLogTypes = new ArrayList<>();
			assignLogTypes.add(WorkflowLog.TASK_ASSIGN);
			List<WorkflowLog> wfAssignLogs = WorkflowLogManagerUtil.getWorkflowLogsByWorkflowInstance(
					themeDisplay.getCompanyId(), instance.getWorkflowInstanceId(), assignLogTypes, -1, -1,
			...
			List<String> transitionNames = WorkflowTaskManagerUtil.getNextTransitionNames(themeDisplay.getCompanyId(),
					themeDisplay.getUserId(), task.getWorkflowTaskId());
			try {
			...
			if (themeDisplay.getUser().getRoles().contains(role)) {
				...
				WorkflowTaskManagerUtil.assignWorkflowTaskToUser(themeDisplay.getCompanyId(), themeDisplay.getUserId(),
						task.getWorkflowTaskId(), themeDisplay.getUserId(), "", new Date(),
				...
				for (String transitionName : transitionNames) {
					...
					WorkflowTaskManagerUtil.completeWorkflowTask(themeDisplay.getCompanyId(), themeDisplay.getUserId(),
							task.getWorkflowTaskId(), transitionName, comments, instance.getWorkflowContext());
				}
			}
