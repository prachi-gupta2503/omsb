...
<div class="tab-pane fade" id="pills-refused"
	aria-labelledby="pills-refused-tab">
	<aui:form action="${filterLogProcedures}" name="pills-refused-tab_form">
		<aui:input name="tab" type="hidden" value="pills-refused-tab" />
		<aui:input name="status" type="hidden"
			value="<%=OmsbViewProceduresSupervisorWebPortletKeys.STATUS_REFUSE%>" />
		<div class="row m-0">
	...
	<div class="omsb-list-view table-responsive">
		<table id="refusedProceduresTable" class="display omsb-datatables">
			<caption></caption>
			<thead>
				<tr>
			...
			<tbody>
				<c:forEach items="${refusedLoggedProcedures}" var="loggedProcedure">
					<portlet:renderURL var="editProcedureURL">
						...
						<portlet:param name="tab" value="pills-refused-tab" />
					</portlet:renderURL>
					<portlet:actionURL
								...
								<ul class="dropdown-menu">
									...
									<c:if test="${permissionChecker.hasPermission(scopeGroupId, portletDisplay.rootPortletId, portletDisplay.resourcePK, DELETE_LOG_PROCEDURE)}">
										<li><a href="javascript:void(0)"
											...
											data-target="#deleteModal" data-toggle="modal" data-tab="pills-refused-tab"><i
...
$(document).ready(function(){
	$('#refusedProceduresTable').DataTable({
		dom: 'Bfrtip',
		buttons: [
    		{
              extend: 'colvis',
