...
<div class="tab-pane fade" id="pills-notpassed"
	aria-labelledby="pills-notpassed-tab">
	<aui:form action="${filterLogProcedures}"
		name="pills-notpassed-tab_form">
		<aui:input name="tab" type="hidden" value="pills-notpassed-tab" />
		<aui:input name="status" type="hidden"
			value="<%=OmsbViewProceduresSupervisorWebPortletKeys.STATUS_NOT_PASS%>" />
		<div class="row m-0">
			...
			<div class="col-lg-6 col-md-6 col-sm-12">
				<div class="form-group">
					<aui:input name="endDate" id="endDate" label="to-date" type="text"
	...
	<div class="omsb-list-view table-responsive">
		<table id="notPassedProceduresTable" class="display omsb-datatables">
			<caption></caption>
			<thead>
				<tr>
			...
			<tbody>
				<c:forEach items="${notPassLoggedProcedures}" var="loggedProcedure">
					<portlet:renderURL var="editProcedureURL">
						...
						<portlet:param name="tab" value="pills-notpassed-tab" />
					</portlet:renderURL>
								...
								<ul class="dropdown-menu">
									<c:if test="${permissionChecker.hasPermission(scopeGroupId, portletDisplay.rootPortletId, portletDisplay.resourcePK, DELETE_LOG_PROCEDURE)}">
										<li><a href="javascript:void(0)"
											...
											data-target="#deleteModal" data-toggle="modal" data-tab="pills-notpassed-tab"><i
...
<script>
	$(document).ready(function() {
		$('#notPassedProceduresTable').DataTable({
			dom: 'Bfrtip',
			buttons: [
	    		{
	              extend: 'colvis',
