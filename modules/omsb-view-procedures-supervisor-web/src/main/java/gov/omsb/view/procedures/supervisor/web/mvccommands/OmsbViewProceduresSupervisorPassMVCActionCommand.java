...
@Component(immediate = true, property = {
		...
		"mvc.command.name="
				+ OmsbViewProceduresSupervisorWebPortletKeys.PASS_PRODCEDURE }, service = MVCActionCommand.class)
public class OmsbViewProceduresSupervisorPassMVCActionCommand implements MVCActionCommand {
	...
	public boolean processAction(ActionRequest actionRequest, ActionResponse actionResponse) throws PortletException {
		...
		String traineeLoggedProcedureDetailsIds = ParamUtil.getString(actionRequest,
				OmsbViewProceduresSupervisorWebPortletKeys.TRAINEE_LOGGED_PROCEDURE_DETAILS_IDS_FOR_PASS, "");

		ThemeDisplay themeDisplay = (ThemeDisplay) actionRequest.getAttribute(WebKeys.THEME_DISPLAY);

		Map<Locale, String> supervisorCommentsMap = LocalizationUtil.getLocalizationMap(actionRequest,
				OmsbViewProceduresSupervisorWebPortletKeys.PASS_COMMENT);

		updateDetails(traineeLoggedProcedureDetailsIds, supervisorCommentsMap,
				OmsbViewProceduresSupervisorWebPortletKeys.STATUS_PASS, themeDisplay, actionRequest);
	...
	private static final Log _logger = LogFactoryUtil
			.getLog(OmsbViewProceduresSupervisorPassMVCActionCommand.class.getName());
