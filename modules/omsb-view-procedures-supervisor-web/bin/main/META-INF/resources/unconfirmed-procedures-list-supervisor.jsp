...
<div class="tab-pane fade" id="pills-unconfirmed"
	aria-labelledby="pills-unconfirmed-tab">
	<div class="omsb-list-view table-responsive">
		<table id="unconfirmedProceduresTable" class="display omsb-datatables">
			<caption></caption>
			<thead>
				<tr>
					<th class="no-sort">
						<div class="custom-control custom-checkbox">
							<input type="checkbox"
								...
								id="checkAllUnconfirmed" name="checkAll"> <label
								class="custom-control-label" for="checkAll"></label>
						</div>
					</th>
			...
			<tbody id="unconfirmedProcedureBody">
				<c:set var="isUnconfirmStatusPresent" value="${false}" />
				<c:forEach items="${unconfirmedTraineeLoggedProcedureDetailsDTOs}"
					...
					<liferay-portlet:renderURL var="viewProcedure">
						...
						<liferay-portlet:param name="tab" value="pills-unconfirmed-tab" />
								...
								<ul class="dropdown-menu">
									...
									<c:if test="${permissionChecker.hasPermission(scopeGroupId, portletDisplay.rootPortletId, portletDisplay.resourcePK, PASS_LOG_PROCEDURE)}">
										<li><a href="javascript:void(0)"
											class="dropdown-item ucOpenPassModel"
											id="${traineeLoggedProcedureDetails.traineeLoggedProcedureDetailsId}"
									...
									<c:if test="${permissionChecker.hasPermission(scopeGroupId, portletDisplay.rootPortletId, portletDisplay.resourcePK, NOT_PASS_LOG_PROCEDURE)}">
										<li><a href="javascript:void(0)"
											class="dropdown-item ucOpenNotPassModel"
											id="${traineeLoggedProcedureDetails.traineeLoggedProcedureDetailsId}"
											data-id="${traineeLoggedProcedureDetails.traineeLoggedProcedureDetailsId}"
									...
									<c:if test="${permissionChecker.hasPermission(scopeGroupId, portletDisplay.rootPortletId, portletDisplay.resourcePK, REFUSE_LOG_PROCEDURE)}">
										<li><a href="javascript:void(0)"
											class="dropdown-item ucOpenRefuseModel"
											id="${traineeLoggedProcedureDetails.traineeLoggedProcedureDetailsId}"
											data-id="${traineeLoggedProcedureDetails.traineeLoggedProcedureDetailsId}"
	...
	<div class="omsb-card bottom-backbtn-wrap">
		<c:if test="${permissionChecker.hasPermission(scopeGroupId, portletDisplay.rootPortletId, portletDisplay.resourcePK, PASS_LOG_PROCEDURE)}">
			<a class="btn omsb-bc-red-button" href="#" id="ucPassAll"
				data-target="#passModalCenter" data-toggle="modal"><liferay-ui:message
					key="pass-all-selected" /></a>
		</c:if>
		<c:if test="${permissionChecker.hasPermission(scopeGroupId, portletDisplay.rootPortletId, portletDisplay.resourcePK, NOT_PASS_LOG_PROCEDURE)}">
			<a class="btn omsb-bc-red-button" href="#" id="ucNotPassAll"
		...
		<c:if test="${permissionChecker.hasPermission(scopeGroupId, portletDisplay.rootPortletId, portletDisplay.resourcePK, REFUSE_LOG_PROCEDURE)}">
			<a class="btn omsb-bc-red-button"
				href="#" id="ucRefuseAll" data-target="#refuseModalCenter"
...
$(document).ready(function(){
	$('#unconfirmedProceduresTable').DataTable({
	    "sDom": 'Rfrtlip',
		"order": [],
	    "columnDefs": [ {
	      "targets"  : 'no-sort',
...
$(document).on("click",".ucOpenPassModel", function(){
	$('#<portlet:namespace/>traineeLoggedProcedureDetailsIdsForPass').val(''+($(this).data('id')));
	$('#<portlet:namespace/>tab').val('pills-unconfirmed-tab');
});

$(document).on("click",".ucOpenNotPassModel", function(){
	$('#<portlet:namespace/>traineeLoggedProcedureDetailsIdsForNotPass').val(''+($(this).data('id')));
	$('#<portlet:namespace/>tab').val('pills-unconfirmed-tab');
});

$(document).on("click",".ucOpenRefuseModel", function(){
	$('#<portlet:namespace/>traineeLoggedProcedureDetailsIdsForRefuse').val(''+($(this).data('id')));
	$('#<portlet:namespace/>tab').val('pills-unconfirmed-tab');
});

$('#checkAllUnconfirmed').click(function() {
   var checked = $(this).prop('checked');
   $('#unconfirmedProcedureBody input:checkbox[name=procedureCheckbox]').prop('checked', checked);
 });
 
$(document).on("click","#ucPassAll", function(){
	var selectedIds=[];
	$("#unconfirmedProcedureBody input:checkbox[name=procedureCheckbox]:checked").each(function(){
	    var $this = $(this);
	    if($this.is(":checked")){
...
$(document).on("click","#ucNotPassAll", function(){
	var selectedIds=[];
	$("#unconfirmedProcedureBody input:checkbox[name=procedureCheckbox]:checked").each(function(){
	    var $this = $(this);
	    if($this.is(":checked")){
...
$(document).on("click","#ucRefuseAll", function(){
	var selectedIds=[];
	$("#unconfirmedProcedureBody input:checkbox[name=procedureCheckbox]:checked").each(function(){
	    var $this = $(this);
	    if($this.is(":checked")){
