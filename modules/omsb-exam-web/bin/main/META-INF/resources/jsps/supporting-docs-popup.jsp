<!-- Supporting Document Popup -->
	<div class="modal fade omsb-modal" id="addsupportingdocument" tabindex="-1" role="dialog"
		...
		<script>
			...
			function validateAndSaveFormData(action) {
				
				var errorMessages = [];
				var comment = $("#comment").val()
				if (!comment) {
					errorMessages.push("<liferay-ui:message key='please-enter-comment' />");
					$("#errorContainer-comment").textContent = "<liferay-ui:message key='please-enter-comment' />";
				}else {
					$("#errorContainer-comment").textContent = "";
				}
				if (errorMessages.length > 0) {
					event.preventDefault();
			} else{
				saveSupportingDocs(action)
				}
