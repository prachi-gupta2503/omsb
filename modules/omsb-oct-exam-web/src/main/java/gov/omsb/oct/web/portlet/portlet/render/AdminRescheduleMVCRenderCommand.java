 ...
 * "mvc.command.name="+MVCCommandNames.ADMIN_RESCHEDULE_RENDER, }, service =
 * MVCRenderCommand.class) public class AdminRescheduleMVCRenderCommand
 ...
 * ParamUtil.getLong(renderRequest, "rescheduleId"); long workflowTaskId =
 ...
 * (PortalException e) {
 * logger.error("Exception while getting the oct exam reschedule entry :::: ",
 * e); } List<OCTExamAppealStatus> rescheduleStatusList = new ArrayList<>();
 * List<OCTExamAppealStatus> statusList =
 * getExamRescheduleStatusByRescheduleId(themeDisplay, id); for
 * (OCTExamAppealStatus status: statusList) { String url =
 * themeDisplay.getPortalURL() + LRObjectURL.OC_EXAM_RESCHEDULE_DOCUMENT_URL +
 * CommonConstants.SCOPES + StringPool.SLASH + themeDisplay.getScopeGroupId() +
 * StringPool.QUESTION + "filter="+
 * OCTExamConstants.FIELD_OC_EXAM_RESCHEDULE_STATUS_ID +
 ...
 * rescheduleStatusList.add(status); } CancellationDetail detail = new
 * CancellationDetail(); detail =
 * octExamCancellationUtil.getWorkflowData(themeDisplay, detail, id,
 * OmsbOctExamWebPortletKeys.RESCHEDULE_OBJECT_ERC); //List<String> trNames =
 * CustomWorkflowTaskUtil.getTransitionNames(themeDisplay, workflowTaskId);
 * renderRequest.setAttribute("statusList", rescheduleStatusList);
 * renderRequest.setAttribute("id", id);
 * renderRequest.setAttribute("assignedToMe", detail.isAssignedToMe());
 * renderRequest.setAttribute("instanceId", detail.getWorkflowInstanceId());
 * renderRequest.setAttribute("workflowTaskId", detail.getWorkflowTaskId());
 * renderRequest.setAttribute("trNames", detail.getTransitionNames());
 * renderRequest.setAttribute("examTitle", examTitle);
 * 
 * return OmsbOctExamWebPortletKeys.ADMIN_RESCHEDULE_JSP; }
 * 
 * public List<OCTExamAppealStatus>
 * getExamRescheduleStatusByRescheduleId(ThemeDisplay themeDisplay, long id) {
 * 
 * logger.info("------getExamAppealStatusByAppealId----"); String url =
 * themeDisplay.getPortalURL() + LRObjectURL.OC_EXAM_RESCHEDULE_STATUS_URL +
 * CommonConstants.SCOPES + StringPool.SLASH + themeDisplay.getScopeGroupId() +
 * StringPool.QUESTION + "filter="+OCTExamConstants.FIELD_OC_EXAM_RESCHEDULE_ID
 * + URLEncoder.encode(" eq " + id, StandardCharsets.UTF_8);
 * logger.info("url == " + url); String response = omsbCommonApi.getData(url);
 * logger.info("------getExamRescheduleStatusByRescheduleId----"+response);
 * OCTExamAppealStatusItem statusItems = null; if
 * (Validator.isNotNull(response)) { statusItems =
 * CustomObjectMapperUtil.readValue(response, OCTExamAppealStatusItem.class); }
 * if (Validator.isNotNull(statusItems) &&
 ...
 * LogFactoryUtil.getLog(AdminRescheduleMVCRenderCommand.class); }
 */