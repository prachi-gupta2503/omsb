...
@Component(immediate = true, property = { "javax.portlet.name=" + OmsbOctExamWebPortletKeys.OMSBOCTEXAMWEB,
		"mvc.command.name=" + MVCCommandNames.SAVE_OCT_TRAINING_SITE_DETAILS }, service = MVCResourceCommand.class)
public class SaveTrainingSiteDetailsMVCResourceCommand extends BaseMVCResourceCommand {

	private static final Log _log = LogFactoryUtil.getLog(SaveTrainingSiteDetailsMVCResourceCommand.class);

	@Override
	public void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws Exception {
		_log.info("Entry into SaveTrainingSiteDetailsMVCResourceCommand ::");
		...
		if (Validator.isNull(value_ar_SA) || Validator.isNull(value_en_US)) {
			...
			try {
				ListTypeDefinition listTypeDefinition = ListTypeDefinitionLocalServiceUtil
						.getListTypeDefinitionByExternalReferenceCode(OmsbOctExamWebPortletKeys.TRAINING_SITE_PL_ERC,
