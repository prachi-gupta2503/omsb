...
<portlet:actionURL
	name="<%=MVCCommandNames.EDIT_MEMBERSHIP_DETAILS_ACTION%>"
	var="editMemberDetailsActionURL">
...
<%
	...
	List<CustomCountryItems> country = (List<CustomCountryItems>) renderRequest
			.getAttribute("customCountryItemList");
	Gson gson = new Gson();
			...
			<div class="container">
				<div class="omsb-card">
					<div class="omsb-page-top-info mb-4">
						<div class="pagetitle">
							<liferay-ui:message
								key="ec-member-request-add-edit-member-details" />
						</div>
					</div>
					<aui:form action="${editMemberDetailsActionURL}" method="post"
						name="addMemberDetails" enctype="multipart/form-data">
						<aui:input type="hidden" name="workflowTaskDetails" />
						<div class="row">
							<div class="col-md-4 col-sm-6 col-xs-6">
								<div class="form-group">
									<aui:input name="bankDetailsId"
										value="${bankDetails.bankDetailsId }" type="hidden"></aui:input>
									<aui:input type="text" id="bankName"
										label="ec-member-request-bank-name" name="bankName"
										value="${bankDetails.bankName}" class="form-control">
							...
							<div class="col-md-4 col-sm-6 col-xs-6">
								<div class="form-group">
									<aui:input type="text" name="accountNumber" id="accountNumber"
										label="ec-member-request-account-number"
										value="${bankDetails.accountNumber}" class="form-control">
										<aui:validator name="required" />
									</aui:input>
								</div>
							</div>
							<div class="col-md-4 col-sm-6 col-xs-6">
								<div class="form-group">
									<aui:input type="text" name="bankBranch" id="bankBranch"
										label="ec-member-request-branch-name"
										value="${bankDetails.bankBranch}" class="form-control">
									...
									<div class="form-group">
										<div class="custom-file required-asterisk mb-3">
											<aui:input type="file" class="custom-file-input"
												id="passportId" label="ec-member-request-passport"
												name="passportId"
												value=" ${documentInfoPassport.dFFileName}"
												onChange="validateFile(this,'passportdetailsId')">
											</aui:input>
											<label class="custom-file-label"
												for="<portlet:namespace/>passportId">${documentInfoPassport.dFFileName}</label>
											<small id="passportdetailsId"  style="color: #dc3545;"></small>
										</div>
									</div>
								</div>
								<div class="col-md-6 col-sm-6 col-xs-12">
									<div class="form-group">
										<div class="custom-file required-asterisk mb-3">
											<aui:input type="file" class="custom-file-input"
												label="ec-member-request-national-id"
												onChange="validateFile(this,'nationaldetailsId')"
												id="nationalId" name="nationalId"
												value="${documentInfonational.dFFileName}">
											</aui:input>
											<label class="custom-file-label"
												for="<portlet:namespace/>nationalId">
												${documentInfonational.dFFileName} </label>
											<small id="nationaldetailsId" style="color: #dc3545;"></small>
						...
						<div class="bottom-backbtn-wrap mt-4">
							<!-- button class="btn omsb-bc-red-button" type="submit" title="Save">Save</button -->
							<c:choose>
				...
				<ul class="omsb-comments-list pb-3">
					<li>
						...
						<c:if test="${not empty memberDetails.commentsSections}">
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<button type="button" class="close reset-on-cancel" data-dismiss="modal"
					aria-label="Close">
			...
			<aui:form action="#" name="ec_member_education_details" method="post"
				...
				<div class="modal-body">
					<div class="row">
						...
						<div class="col-md-4 col-sm-4 col-xs-12">
							<div class="form-group">
								<%-- <input type="text" name="<portlet:namespace/>title" id="title" placeholder="EC VP" class="form-control">  --%>
								<aui:select class="form-select form-control"
						...
						<div class="col-md-6 col-sm-6 col-xs-12">
							<div class="form-group">
								<aui:input cssClass="form-control" type="text"
									placeholder="Enter GPA" name="gpa" id="gpa" label="ec-member-request-gpa">
									<aui:validator name="required"></aui:validator>
								</aui:input>
						...
						<div class="col-md-6 col-sm-6 col-xs-12">
							<div class="form-group">
								<aui:select class="form-select form-control"
									...
									<option><liferay-ui:message key="ec-member-request-select" /></option>
									<aui:validator name="required"></aui:validator>
								</aui:select>
							</div>
						</div>
							...
							<div class="form-group">
								<div class="custom-file required-asterisk mb-3">
									<aui:input class="custom-file-input" type="file"
										...
										name="qualificationDocument" label="ec-member-request-qualification-document">
									</aui:input>
									<label class="custom-file-label"
				...
				<div class="modal-footer">
					<aui:button class="btn omsb-bc-red-button" type="button"
						value="ec-member-request-save" onClick="addMemberEducationDetails()"></aui:button>
					<a class="btn omsb-btn omsb-bg-red-button reset-on-cancel" data-dismiss="modal"
						value="label-submit-cancel"><liferay-ui:message key="ec-member-request-cancel"/></a>
				</div>
			</aui:form>
		</div>
	</div>
...
$(document).ready(function() {
	 $.ajax({
		    type: "GET",
		    url: "${portalURL}/o/headless-admin-list-type/v1.0/list-type-definitions/${universitylistTypeDefinitionId}?p_auth=" + Liferay.authToken,
		    contentType: "application/json; charset=utf-8",
       	 }).done(function(response){
       	  for(let data of response.listTypeEntries){
       		universityName=data.name;
       		
       	    $("#<portlet:namespace/>issuingAuthorityName").append(new Option(universityName,universityName));
       		} 
      	  })
	 $.ajax({
		    type: "GET",
		    url: "${portalURL}/o/headless-admin-list-type/v1.0/list-type-definitions/${qualificationListTypeDefinitionId}?p_auth=" + Liferay.authToken,
		    contentType: "application/json; charset=utf-8",
         }).done(function(response){
        	  for(let data of response.listTypeEntries){
        		qualificationName=data.name;
        		
        	    $("#<portlet:namespace/>title").append(new Option(qualificationName,qualificationName));
        		} 
       	  });
	...
	var label = item.id;
	var value =item.nationality;
	 $("#<portlet:namespace/>issuingAuthorityCountryName").append(new Option(value,label));
	console.log(value);
	});
	...
	function validateForm(ec_member_education_details){
		var liferayForm = Liferay.Form.get(ec_member_education_details);
		if(liferayForm){
				var validator = liferayForm.formValidator;
				 validator.validate();
				 var hasErrors = validator.hasErrors();
				 if(hasErrors && $('#<portlet:namespace/>qualificationDocument').val()==''){
						validator.focusInvalidField();
						$('#qualificationdetailsId').text('Qualification document is required');
						return false;
					}else if($('#<portlet:namespace/>qualificationDocument').val()==''){
						validator.focusInvalidField();
						$('#qualificationdetailsId').text('Qualification document is required');
						return false;
					}
	...
	function bindFormDataJson(formObj){
		...
		function addMemberEducationDetails(){
			let isValid = validateForm('<portlet:namespace/>ec_member_education_details');
		...
		if(isValid){
		       ...
		       // var oTable = $('#educationDetailTable').DataTable( );
					$.ajax({
		    			type: "post",
					    url: url,
					    data: formdata,
		    			...
		    			var refreshedDataFromTheServer = getEducationDetails();
			    ...
			    },
			    {
				     ...
				     title:"<liferay-ui:message key='ec-member-request-document'/>" , 
				      render: function(data, type, row) {
				    	        ...
				    	        return `<a href="`+data+`" class="dropdown-item">
			    ...
			    $.ajax({
			          ...
			          success: function(response) {
			        	if (response) {
			                ...
			                $("#<portlet:namespace/>issuingAuthorityName").val(response.issuingAuthorityName);
			...
			function deleteEducationDetails(id){
			  ...
			  $.ajax({
		  		    type: "DELETE",
		  		    url: "<%=portalURL%>/o/c/educationdetails/"+id+"?p_auth=" + Liferay.authToken,
		  		    contentType: "application/json; charset=utf-8",
		  		  }).done(function(response) {
		  			let table = $('#educationDetailTable').DataTable();
