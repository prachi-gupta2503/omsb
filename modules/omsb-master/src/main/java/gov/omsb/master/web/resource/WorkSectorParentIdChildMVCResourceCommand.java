...
@Component(immediate = true, property = { "javax.portlet.name=" + OmsbMasterPortletKeys.OMSBMASTER,
		"mvc.command.name=" + MVCCommandNames.WORK_SECTOR_TYPE_PARENT_ID_1_RESOURCE_COMMAND }, service = MVCResourceCommand.class)
public class WorkSectorParentIdChildMVCResourceCommand extends BaseMVCResourceCommand {

	@Override
	protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
			throws Exception {

		logger.info("WorkSectorParentIdChildMVCResourceCommand Started () () ()");
		
		JSONArray jsonArray = JSONFactoryUtil.createJSONArray();
		ThemeDisplay themeDisplay = (ThemeDisplay) resourceRequest.getAttribute(WebKeys.THEME_DISPLAY);
		long workSectorTypeParentId = ParamUtil.getLong(resourceRequest, "workSectorTypeParentId");
		logger.info("workSectorTypeParentId:"+workSectorTypeParentId);
		WorksectorsItems worksectorsItems = getWorkSectorByParentId(themeDisplay, workSectorTypeParentId);
	...
	public WorksectorsItems getWorkSectorByParentId(ThemeDisplay themeDisplay, long workSectorTypeParentId)
		...
		String workSectorMasterUrl =  themeDisplay.getPortalURL() + MVCCommandNames.WORK_SECTOR_TYPE_URL  + CommonConstants.SCOPES + StringPool.SLASH + 
				...
				+URLEncoder.encode(" eq " + workSectorTypeParentId, StandardCharsets.UTF_8) + "&sort=workSector:asc&pageSize=0";
	...
	private static final Log logger = LogFactoryUtil.getLog(WorkSectorParentIdChildMVCResourceCommand.class);
}
