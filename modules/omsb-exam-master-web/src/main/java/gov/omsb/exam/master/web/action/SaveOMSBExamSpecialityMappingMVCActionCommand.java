...
import gov.omsb.exam.master.web.constants.OmsbExamMasterConstants;
	...
	protected void doProcessAction(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		try {
			...
			if (Validator.isNotNull(listTypeDefinition)) {
				ListTypeEntry listTypeEntry = ListTypeEntryLocalServiceUtil.addListTypeEntry(null,
						themeDisplay.getUserId(), listTypeDefinition.getListTypeDefinitionId(), key, nameMap);
				subSpeciality = listTypeEntry.getListTypeEntryId();
			}

			Map<String, Serializable> values = new HashMap<>();
			values.put(OmsbExamMasterConstants.OMSB_EXAM_SPECIALITY_MASTER, speciality);
			values.put(OmsbExamMasterConstants.OMSB_EXAM_SUB_SPECIALITY_MASTER, subSpeciality);
			...
			if (Validator.isNull(objectEntry)) {
				SessionErrors.add(actionRequest, OmsbExamMasterWebPortletKeys.SET_SPECIALITY_MAPPING_FORM_ERROR);
			}
		} catch (Exception e) {
			SessionErrors.add(actionRequest, OmsbExamMasterWebPortletKeys.SET_SPECIALITY_MAPPING_FORM_ERROR);
			logger.error(e);
