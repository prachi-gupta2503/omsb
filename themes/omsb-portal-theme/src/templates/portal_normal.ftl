...
<#assign
		userProfilefileEntryId=themeDisplay.getUser().getPortraitId()
		...
		imageUrl="${images_folder}/svg/profile.svg"
		/>
<#if userProfilefileEntryId!=0>
  <#attempt>
	<#assign fileEntry=dLFileEntryLocalService.getFileEntry(userProfilefileEntryId)/>
	<#assign
		imageUrl=themeDisplay.getPortalURL() + "/c/document_library/get_file?uuid=" + fileEntry.getUuid() + "&groupId=" + fileEntry.getGroupId()
		/>
		<#recover>
 </#attempt>
</#if>
								...
								<div class="rightbar">
									<div class="buttons-wrap">
										...
										<#if themeDisplay.isSignedIn()>
											...
											<div class="button-col">
												<button type="button" class="profile-dtls dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="User Profile"><img class="profile-pic" src="${imageUrl}"></button>
